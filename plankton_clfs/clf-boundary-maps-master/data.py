...
def LoadSegmentationData():
    data = np.loadtxt("data/segmentation/orig/segmentation.txt", delimiter=",",
                      comments='#')
    y = data[:, 0].astype(int)
    X = data[:, 1:]
    X = preprocessing.MinMaxScaler().fit_transform(X)
...
def download_cifar10(path):
    ...
    batches_folder = os.path.join(path, 'cifar-10-batches-py/')
    if os.path.exists(batches_folder):
        return

    # Folders dont exist, so need at least to extract the tar file
    file_name = "cifar-10-python.tar.gz"
    file_path = os.path.join(path, file_name)

    # Tar file also doesn't exist, download it
...
def LoadCifar10(data_dir="data/cifar10/"):
    ...
    for i in range(1, 6):
        curr_file = "data_batch_{}".format(i)
        data_dic = unpickle(os.path.join(data_dir, curr_file))
        if i == 1:
